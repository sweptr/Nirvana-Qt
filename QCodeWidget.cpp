...
const QString DefaultFont = "Courier New";
...
QCodeWidget::QCodeWidget(QWidget *parent)
	...
	emulateTabs_ = 0;
	firstChar_ = 0;
	fixedFontWidth_ = viewport()->fontMetrics().maxWidth(); // TODO(eteran): properly detect
	...
	// set default size
	resize(DefaultWidth * viewport()->fontMetrics().maxWidth(),
...
void QCodeWidget::setFont(const QFont &font) {
	...
	if (font.fixedPitch()) {
		fixedFontWidth_ = viewport()->fontMetrics().maxWidth();
	} else {
		fixedFontWidth_ = -1;
	}
}
...
void QCodeWidget::redisplayLine(QPainter *painter, int visLineNum, int leftClip, int rightClip, int leftCharIndex,
	 ...
	 */
	int stdCharWidth = viewport()->fontMetrics().maxWidth();
...
void QCodeWidget::drawCursor(QPainter *painter, int x, int y) {
	 ...
	 */
	const int cursorWidth = (viewport()->fontMetrics().maxWidth() / 3) * 2;
	...
	if (cursorStyle_ == CARET_CURSOR) {
		...
		const int right = x + viewport()->fontMetrics().maxWidth();
		path.moveTo(x, y);
		path.lineTo(right, y);
		path.moveTo(right, y);
		path.lineTo(right, bot);
...
void QCodeWidget::TextInsertAtCursor(const char *chars, bool allowPendingDelete, bool allowWrap) {
	...
	int wrapMargin = wrapMargin_ != 0 ? wrapMargin_ : viewport()->width() / viewport()->fontMetrics().maxWidth();
...
void QCodeWidget::redrawLineNumbers(bool clearAll) {
	...
	int charWidth  = viewport()->fontMetrics().maxWidth();
	QRectF clipRect;
...
void QCodeWidget::autoScrollTimeout() {
	...
	int cursorX;
	int y;
	const int fontWidth = viewport()->fontMetrics().maxWidth();
	const int fontHeight = viewport()->fontMetrics().ascent() + viewport()->fontMetrics().descent();
